...
export default function BaseEdge(props: BaseProps) {
	...
	if (!props.baseEdgeProps.targetNode.inputHandle) {
		createRenderEffect(() => {
			...
			const dy =
				props.baseEdgeProps.targetNode.input.y -
				props.baseEdgeProps.sourceNode.output.y;
			let newValue = 0;
			const corner =
				(Math.atan2(
					props.baseEdgeProps.targetNode.height,
					props.baseEdgeProps.targetNode.width,
				) *
					180) /
				Math.PI;
