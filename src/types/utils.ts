...
type HexDigit<
	T extends
		...
		| "F",
> = T;
export type HexColor<T extends string> = T extends `#${HexDigit<
	infer D1
>}${HexDigit<infer D2>}${HexDigit<infer D3>}${infer Rest1}`
	? Rest1 extends ``
		? T // three-digit hex color
		: Rest1 extends `${HexDigit<infer D4>}${HexDigit<infer D5>}${HexDigit<
				infer D6
		  >}`
